...
export default {
    ...
    data() {
        return {
            ...
            titleRight: 'List Debit Vouchers',
            is_update: false,
            submit_text: 'Create',
            loading:false,
            form: {
    ...
    mounted() {
        ...
        this.$store.dispatch(SET_BREADCRUMB, [{title: "Debit Voucher"}]);
    },
    methods: {
        ...
        onCellStopEditing(event) {
            ...
            this.form.amount = parseFloat(totalAmount);
        ...
        onSubmit(evt) {
            ...
            this.form={...this.form,transactions:this.rowData};
                ...
                ApiService.post('/api/debit-voucher/create', this.form).then((res) => {
                    this.$bvToast.toast(res.data.message, {
                        title: `Debit Voucher Created Successfully...`,
        ...
        redirectToIndex() {
            this.$router.push({name: 'debit_voucher_index'});
        },
        onContext(event) {
            console.log(event);
        },
