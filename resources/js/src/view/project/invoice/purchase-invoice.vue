                ...
                <KTCard ref="preview" v-bind:title="title" v-if="true">
                    ...
                    <template v-slot:body>
                        <div>
                            ...
                            <b-row>
                                <b-col v-for="(item,index) in form.types" :key="index">
                                    <b-row>
                                        <b-col md="8">
                                            <label>{{ item.name }}
                                            <span v-if="item.payment_type=='percent'">(%)</span>
                                            <span v-if="item.payment_type=='fixed'">(Rs)</span>
                                            <span v-if="item.payment_type=='per-bag'">(Per Bag)</span>
...
<!--                                            <b-input v-if="item.payment_type=='percent'" disabled type="number" v-model="form.types[index].total"></b-input>-->
<!--                                        </b-col>-->
                                        ...
                                        <b-col md="4">
                                            <b-form-group
                                                ...
                                                <b-form-radio-group
                                                    v-model="form.types[index].expense_status='dr'"
...
export default {
    ...
    methods: {
        ...
        onCellStopEditing(event) {
            ...
            ApiService.get('/api/deal/fetch-deductions?name='+rowNode.data.deduction).then((res) => {
                ...
                if (checkif==true){
                    ...
                    this.form.deductions.push(
                        {commodity:res.data.commodity,id:res.data.id,items:res.data}
                    )
                }else {
                 this.form.deductions.push(
                     {commodity:res.data.commodity,id:res.data.id,items:res.data}
            ...
            console.log('GriD Api Data',items)
            // }
        ...
        onSubmit: function () {
            ...
            ApiService.post('/api/purchase-deal/create-invoice', this.form).then((res) => {
