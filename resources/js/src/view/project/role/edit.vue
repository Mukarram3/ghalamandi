                                ...
                                <b-form-group id="input-group-1" label="Permissions :*" label-for="input-1" description="">
                                    <table>
                                        ...
                                        <tr v-for="option in options">
                                            <td><input type="checkbox" v-model="selected" @click="select" :value="option.id"></td>
                                            <td> {{ option.name }}</td>
                                        </tr>
                                    </table>
                                </b-form-group>
    ...
    export default {
        ...
        methods: {
            ...
            selectAll: function() {
                ...
                if (this.allSelected) {
                    this.options.map((option) => {
                        this.selected.push(option.id.toString());
                    });
                }
            },
            select: function() {
            ...
            getEditDataFromApi: function () {
                ApiService.get('/api/role/edit', this.$route.params.id).then((res) => {
